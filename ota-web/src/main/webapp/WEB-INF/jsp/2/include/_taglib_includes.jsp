...
<%--市散装办员工--%>
<c:set var="loginedMemberCity"
       value="${loginedMember and OrganizationTypeCITY eq sessionScope.sessionInfo.organizationType}"/>
<%--区散装办员工--%>
...
<%--车企员工--%>
<c:set var="loginedMemberCompany"
       value="${loginedMember and OrganizationTypeCOMPANY eq sessionScope.sessionInfo.organizationType}"/>


<%--超级管理员登录--%>
<%--<c:set var="rootLoginName" value="<%="root"%>"/>--%>
<c:set var="loginedRoot"
       value="${loginedMember and 'root' == sessionScope.sessionInfo.loginName}"/>

<%--
logined  已登录
loginedMember 登录的是员工
loginedDriver 登录的是驾驶员

不同的员工身份
loginedMemberProvince
loginedMemberCity
loginedMemberDistrict
loginedMemberCompany
...
<c:if test="${!logined}"/>
多个条件
<c:if test="${loginedDriver or loginedMemberCompany}"/>
