	  ...
	  dnd: {
		onDragStart: function(node) {
		  ...
		  // logMsg("tree.onDragStart(%o)", node);
			console.info('onDragStart:startNode='+node.data.title);
		  return true;
		},
		onDragStop: function(node) {
		  // This function is optional.
		  // logMsg("tree.onDragStop(%o)", node);
			console.info('onDragStop:stopNode='+node.data.title);
			console.info('onDragStop:stopNode parent='+node.getParent().data.title);
			if(parent.changeParent)
				parent.changeParent(node.data.key,node.getParent().data.key);
		},
		autoExpandMS: 1000,
		...
		onDrop: function(node, sourceNode, hitMode, ui, draggable) {
		  ...
		  //console.info("node:"+node.data.title+",sourceNode:"+sourceNode.data.title+",hitMode:"+hitMode);
