...
<script type="text/javascript"
	src="<%=basePath%>media/2/js/exam/member/signup.js"></script>
</head>
...
<body>
	<div class="block block-two batch_update">
		...
		<form id="data" action="<%=basePath%>exam/member/signup/${examId}/content"
			method="get">
			<div style="display: none">
				<input name="page" id="page" value="${pageBaseDto.page }"> <input
					name="rows" id="rows" value="${pageBaseDto.rows }"> <input
					name="pageNum" id="pageNum" value="${pageBaseDto.pageNum }">
				<input name="searchKey" id="searchKey"
					value="${pageBaseDto.searchKey }"> <input name="status"
					id="status" value="${pageBaseDto.status }"> <input
					name="pageForSignup" id="pageForSignup"
					...
					id="signupStatus" name="signupStatus" value="-1">
				<input id="tid" name="tid" value="${orgId }">
			</div>
		</form>
		<div class="block-body">
			...
			<!-- <div class="operation_part">
				<span>筛选条件：</span> <input type="checkbox"
					...
					name="signupStatusCheckbox">初次培训驾驶员 <input type="checkbox"
					class="checkbox-inline condition_checkBox" value="3"
					name="signupStatusCheckbox">重新培训驾驶员 <input type="checkbox"
					class="checkbox-inline condition_checkBox" value="1"
					name="signupStatusCheckbox">其他驾驶员 <input type="checkbox"
					class="checkbox-inline all_checkBox" value="-1"
					name="signupStatusCheckbox">全部
			</div> -->
			<div class="panel-body">
				<div class="car_company">
					<span>${companyName }</span><input type="checkbox"
						class="select_all">全选
				</div>
				...
				<nav id="paging_nav">
					<ul class="pagination">
						<li><a href="#" aria-label="first"
							class="glyphicon glyphicon-step-backward" alt="第一页" title="第一页"
							onclick="changePageForSignup(1)"></a></li>
						<!-- 第一页不能上一页 -->
						<c:if test="${pageBaseDtoForSignup.pageForSignup==1 }">
							<li><a href="#" aria-label="previous"
								class="glyphicon glyphicon-chevron-left" alt="上一页" title="上一页"></a></li>
						</c:if>
						<c:if test="${pageBaseDtoForSignup.pageForSignup!=1 }">
							<li><a href="#" aria-label="previous"
								class="glyphicon glyphicon-chevron-left" alt="上一页" title="上一页"
						...
						<c:if
							...
							<li><a href="#" aria-label="next"
								class="glyphicon glyphicon-chevron-right" alt="下一页" title="下一页"></a></li>
						</c:if>
						<c:if
							test="${pageBaseDtoForSignup.pageForSignup!=pageBaseDtoForSignup.pageNumForSignup }">
							<li><a href="#" aria-label="next"
								class="glyphicon glyphicon-chevron-right" alt="下一页" title="下一页"
								onclick="changePageForSignup('${pageBaseDtoForSignup.pageForSignup+1}')"></a></li>
						</c:if>
						<li><a href="#" aria-label="last"
							class="glyphicon glyphicon-step-forward" alt="最后一页" title="最后一页"
							onclick="changePageForSignup('${pageBaseDtoForSignup.pageNumForSignup}')"></a></li>
						<li><input type="text" aria-controls="" class="input-custom"
							placeholder="页码" id="page_textForSignup"> <input
							type="button" class="btn-custom btn-middle" value="跳转"
			...
			<button class="btn-custom btn-long submit_btn"
				onclick="getSignupDrivers('<%=basePath%>','${examId }','${orgId }')">提交</button>
		</div>
	</div>
			...
			<div id="pb_top">
				...
				<div class="search_part text-center col-md-6">
					<label id="search"><input type="text" aria-controls=""
						...
						value="${pageBaseDto.searchKey }"> </label> <input type="button"
						class="search_btn" value="搜索" id="search_btn" onclick="submit()">
				</div>
			</div>
			<div id="pb_content">
				<table class="table table-bordered table-striped">
					<thead>
						...
						<tr>
							...
							<th class="text-center" style="cursor: pointer;">所属散装办</th>
				...
				<nav id="paging_nav">
					<ul class="pagination">
						...
						<c:if test="${pageBaseDto.page!=1 }">
							<li><a href="#" aria-label="previous"
								class="glyphicon glyphicon-chevron-left" alt="上一页" title="上一页"
								onclick="changePage('${pageBaseDto.page-1}')"></a></li>
						</c:if>
						<!-- 最后一页没有下一页 -->
						...
						<li><a href="#" aria-label="last"
							class="glyphicon glyphicon-step-forward" alt="最后一页" title="最后一页"
							onclick="changePage('${pageBaseDto.pageNum}')"></a></li>
						<li><input type="text" aria-controls="" class="input-custom"
							placeholder="页码" id="page_text"> <input type="button"
							class="btn-custom btn-middle" value="跳转" id="goto_btn"
							onclick="changePage(-1)"></li>
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						onclick="cancelSignup('${examId}','<%=basePath %>','${orgId }')">确定</button>
				</div>
			</div>
		</div>
	</div>
