 ...
 * @param examId
 */
function getSignupDrivers(basePath, trainingId) {
	var source = "";
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				location.href = basePath + "training/company/" + trainingId
						+ "/drivers/signup";
			} else {
				alert(data.error);
			}
 ...
 * @param basePath
 */
function confirmCancelDriver(trainingId, basePath) {
	$.ajax({
		url : basePath + "training/" + trainingId + "/"
				+ $("#cancelDriverId").val() + "/signup",
		type : 'DELETE',
		success : function(data) {
			if (data.success) {
				location.href = basePath + "training/company/" + trainingId
...
function reloadPage(trainingId, basePath) {
	// 刷新页面
	location.href = basePath + "training/company/" + trainingId
			+ "/drivers/signup";
}